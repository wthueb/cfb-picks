...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
  plop.setGenerator("init", {
    description: "Generate a new package for the cfb-picks Monorepo",
    prompts: [
      {
        type: "input",
        name: "name",
        message: "What is the name of the package? (You can skip the `@cfb-picks/` prefix)",
      },
      {
        type: "input",
        name: "deps",
    ...
    actions: [
      (answers) => {
        if ("name" in answers && typeof answers.name === "string") {
          if (answers.name.startsWith("@cfb-picks/")) {
            answers.name = answers.name.replace("@cfb-picks/", "");
          }
        }
